/**
 * connection.test.ts：KurobridgeConnection 全链路单测（任务书 M2）。
 *
 * 用假 kurobridge-ws 服务端（ws 库真端口，握手校验复刻 KuroAdapter NodeWsServer）驱动：
 * 带子协议握手 / 不带被拒 / hello 逐字段对表 / token 有无 / 应用层 ping / pong 透传 /
 * hello 超时自断重试 / 断线指数退避重连且重发 hello / 三类永久停（hello_ack 拒绝、
 * close 1002、1008）/ 用户 stop 静默 / 未知帧容忍。重连节奏用测试快值（抖动 0）。
 */
import { afterAll, describe, expect, it } from "vitest";
import { WebSocket } from "ws";
import type {
    ConnectionEndedReason,
    KurobridgeConnectionOptions,
    ServerEventMessage,
} from "./connection.js";
import { KurobridgeConnection } from "./connection.js";
import { type HelloAckOkBody, WS_SUBPROTOCOL } from "./protocol/index.js";
import { type FakeKurobridgeServer, startFakeKurobridgeServer, waitFor } from "./test-support.js";

/** 测试快参数（重连/心跳/握手超时全部加速，抖动归零保确定性）。 */
const FAST = {
    pingIntervalMs: 30,
    helloTimeoutMs: 1000,
    retryBaseDelayMs: 20,
...
function makeConnection(
    url: string,
    overrides?: Partial<KurobridgeConnectionOptions>,
): {
    conn: KurobridgeConnection;
    ...
    const conn = new KurobridgeConnection({
        url,
        hello: {
            peerId: "koishi",
            platform: "koishi",
...
function autoAck(server: FakeKurobridgeServer, bindings: string[], serverId = "fake-server"): void {
...
describe("握手", () => {
    it("带子协议握手成功：hello 逐字段对表，ack 后建立并拿到绑定快照", async () => {
        ...
        // ack ok：绑定快照透出
        expect(established[0]?.channelBindings).toEqual(["967493177"]);
        conn.stop();
        void conn0;
    });
    ...
    it("子协议不匹配 → 被拒（复现服务端 handleProtocols 拒连）", async () => {
        const s = await server();
        const result = await new Promise<{ ok: boolean; err?: Error }>((resolve) => {
            // 带错误子协议（handleProtocols 返回 false → 拒绝升级）。
...
describe("心跳与出站", () => {
    it("建立后按 pingIntervalMs 发应用层 ping 帧（服务端只认应用层入帧）", async () => {
        ...
        conn.start();
        await s.nextConnection();
        // FAST.pingIntervalMs=30ms，等 3 个周期
        await waitFor(() =>
    ...
    it("established 才能发送：chat 帧到服务端；未建立时 send 返回 false", async () => {
        ...
        expect(
            conn.send({
                type: "chat",
                body: { channel: "967493177", sender: "群友", content: "你好" },
            }),
        ).toBe(true);
        ...
        expect(chat?.body).toEqual({ channel: "967493177", sender: "群友", content: "你好" });
        conn.stop();
    });
...
describe("重连与停止", () => {
    it("断线后指数退避重连且重发 hello（每次连接都重新握手，UUID 每次新）", async () => {
        const s = await server();
        s.frames.length = 0;
        autoAck(s, ["967493177"]);
        const { conn, established } = makeConnection(`ws://127.0.0.1:${s.port}`);
    ...
    it("hello 超时自断走重试：服务端不回 ack，超时后关闭并退避重连", async () => {
        const s = await server();
        s.frames.length = 0;
        s.handshakes.length = 0;
        s.onClientFrame = () => undefined; // 不应答
        const { conn, established } = makeConnection(`ws://127.0.0.1:${s.port}`, {
            helloTimeoutMs: 80,
        });
        conn.start();
        await s.nextConnection();
        await waitFor(() => (s.handshakes.length >= 2 ? true : null)); // 超时自断后重连
        expect(established).toHaveLength(0);
        expect(s.frames.filter((f) => f.header.type === "hello").length).toBeGreaterThanOrEqual(2);
        conn.stop();
    });
    ...
    it("hello_ack ok:false（auth failed）→ 停止重连 + onEnded(permanent)", async () => {
        ...
        expect(ended[0]?.detail).toContain("auth failed");
        // 等若干退避周期：不应有第二次连接
