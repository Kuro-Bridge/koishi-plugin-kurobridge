/**
 * KurobridgeConnection：kurobridge-ws 客户端连接层（任务书 M2）
 *
 * 状态机 `idle → connecting → establishing → established ↔ backoff → stopped`：
 * - 子协议 `kurobridge-ws.v1` 必带（不带被服务端握手期拒绝）；
 * - open 即发 hello（UUID，超时自断走重试；每次连接/重连都重新握手，UUID 每次新）；
 * - hello_ack 按 id 关联：ok → established + 启动应用层 ping；ok:false → 永久停止
 *   （版本/token 问题不可自愈，peer-guide §7）；
 * - close 1002（协议错误）/ 1008（鉴权失败）→ 永久停止；1001 / 1006 / 网络错误 →
 *   指数退避重连（1s 起 ×2 封顶 30s，±20% 抖动防惊群；重连成功后退避重置）；
 * - 未 established 时 send 丢弃返回 false，不积压；
 * - 未知事件帧忽略、未知请求帧回 `<type>_result` 失败回执、`_result` 结尾不回执
 *   （防乒乓），对齐 peer-guide §5.3 未知帧容忍；
 * - 用户 stop() 静默（清定时器关连接，不触发 onEnded、不再重连）。
 */
...
/** 连接层日志级别（宿主回调面，M4 由适配层透传到 Koishi logger）。 */
...
/** hello body（protocolVersion 固定协议包版本；peerId/platform/version/client 由装配层解析）。 */
export interface KurobridgeHello {
    peerId: string;
    platform: string;
    version: string;
...
export interface KurobridgeConnectionOptions {
    /** KuroBridge 服务端完整地址（ws:// / wss://）。 */
    url: string;
    ...
    /** 退避抖动幅度 0..1（缺省 0.2 = ±20%，peer-guide §7 防惊群；测试快值传 0）。 */
    retryJitter?: number;
    /** 握手成功（ack ok，含绑定快照）。 */
 ...
 * 与协议包 Server→Peer 消息集一一对应；hello_ack 在连接层单独处理，不在此表。
 */
...
export class KurobridgeConnection {
    private readonly opts: KurobridgeConnectionOptions;
    ...
    constructor(opts: KurobridgeConnectionOptions) {
        this.opts = opts;
        this.jitterRatio = opts.retryJitter ?? 0.2;
    }
    ...
    send(message: { type: string; body: unknown; id?: string }): boolean {
        ...
        if (this.state !== "established" || ws === null || ws.readyState !== WebSocket.OPEN) {
            this.log("debug", "未建立连接，丢弃出站帧", { type: message.type });
            return false;
        }
    ...
    private log(level: ConnectionLogLevel, msg: string, extra?: Record<string, unknown>): void {
        this.opts.onLog?.(level, `kurobridge: ${msg}`, extra);
    }
    ...
    private connect(): void {
        ...
        ws.on("error", (err: Error) => {
            ...
            this.log("warn", "连接错误", { err: err.message });
        });
    }
    ...
    private onOpen(): void {
        ...
        this.helloTimer = setTimeout(() => {
            ...
            if (this.state === "establishing") {
                this.log("warn", "等待 hello_ack 超时，关闭重试");
                this.ws?.close();
            }
    ...
    private onText(text: string): void {
        ...
        try {
            ...
            this.log("warn", "非法 JSON 帧，忽略", { text: text.slice(0, 200) });
            return;
        }
        const wire = wireFrameSchema.safeParse(json);
        if (!wire.success) {
            this.log("warn", "帧骨架非法，忽略", { text: text.slice(0, 200) });
            return;
        }
        const { type } = wire.data.header;
        if (type === "hello") {
            this.log("warn", "收到非预期的 hello 帧，忽略");
            return;
        }
        if (type === "hello_ack") {
    ...
    private parseServerFrame(type: string, json: unknown): ServerEventMessage | null {
        ...
        if (msg === null) {
            this.log("warn", "已知帧 body 校验失败，丢弃", { type });
        }
        return msg;
    }

    /** 未知帧容忍（对齐 peer-guide §5.3）：事件帧忽略；请求帧回 `<type>_result` 失败回执；`_result` 结尾不回执（防乒乓）。 */
    private handleUnknownFrame(type: string, id: string | undefined): void {
        if (id === undefined) {
            this.log("debug", "未知事件帧，忽略", { type });
            return;
        }
        if (type.endsWith("_result")) {
            this.log("debug", "未知响应帧，忽略（不回执防乒乓）", { type });
            return;
        }
        this.log("debug", "未知请求帧，回执 unknown frame type", { type });
    ...
    private handleHelloAck(json: unknown): void {
        ...
        if (!parsed.success) {
            this.log("warn", "已知帧 body 校验失败，丢弃", { type: "hello_ack" });
            return;
        }
        if (parsed.data.id !== this.pendingHelloId) {
            this.log("warn", "hello_ack id 与请求不匹配，忽略");
            return;
        }
        if (!parsed.data.body.ok) {
            this.log("error", "握手被拒，停止重连", { reason: parsed.data.body.reason });
        ...
        if (!isProtocolVersionCompatible(this.opts.hello.protocolVersion, ack.protocolVersion)) {
            ...
            this.log("warn", "hello_ack 版本与本地声明主版本不一致", {
                peer: ack.protocolVersion,
                self: this.opts.hello.protocolVersion,
            });
        }
    ...
    private onClose(code: number, reason: string): void {
        ...
        if (code === 1002 || code === 1008) {
            this.log("error", "连接被拒（不可重试错误），停止重连", { code, reason });
            this.rejected = true;
            this.state = "stopped";
        ...
        if (wasEstablished) {
            this.log("warn", "连接断开，准备重连", { code, reason });
        }
        this.scheduleRetry();
    }

    /** 指数退避重连：base * 2^attempt 封顶 retryMaxDelayMs，±jitter 抖动；established 成功清零。 */
    private scheduleRetry(): void {
        ...
        const jitter = this.jitterRatio * (Math.random() * 2 - 1);
        const delay = Math.max(0, Math.round(base * (1 + jitter)));
        this.attempt += 1;
        this.log("debug", "退避后重连", { delay, attempt: this.attempt });
